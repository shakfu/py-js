...
void testassert_assist(t_testassert *x, void *b, long m, long a, char *s)
{
	if (m==ASSIST_INLET) {
		switch (a) {
			case 0: snprintf_zero(s, SPRINTF_MAXLEN, "receive values from the system under test to compare against expectations"); break;
		}
	} 
	else {
		switch (a) {
			case 0: snprintf_zero(s, SPRINTF_MAXLEN, "sends input values to the system under test at loadbang time"); break;
		}
	}
}
