...
## The A_GIMMEBACK method type

This is the basic signature of a gimmeback method provided by JCK in [obex-how-to-get-return-values-from-object_method](https://cycling74.com/forums/obex-how-to-get-return-values-from-object_method#reply-58ed1f37c2991221d9cbadd1) forum question:


```c
t_max_err obex_tester_gimmeback(t_obex_tester *x, t_symbol *s, long ac, t_atom *av, t_atom *rv)
{    
    post("obex tester gimmeback av = %d", atom_getlong(av));
    if (ac && av) {
        // if correctly called, multiplies first long values sent and returns via rv
        atom_setlong(rv, atom_getlong(av) * atom_getlong(av));
    } else {
        // return -1 value in rv
        atom_setlong(rv,-1);
    }
    return MAX_ERR_NONE;
}
...
Best example is in `max-sdk/source/basic/simpeljs/simplejs.c`:
...
As this is a native class exposed to `js` or `v8`, it can be called as follows in javascript:
