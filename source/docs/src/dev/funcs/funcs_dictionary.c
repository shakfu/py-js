...
typedef struct _dictionary_entry {
    t_object e_obj;
    t_symbol* e_key; // redundant with hashtab, but allows getting key during
                     // linklist traversal
    t_atom e_value;
    long e_flags;
} t_dictionary_entry;
...
typedef struct _dictionary {
    ...
    void* d_transaction_mutex; // currently having header inclusion issues this
                               // is a t_systhread_mutex
    char d_use_transaction;    // always lock transaction mutex with low level
                               // dictionary calls
} t_dictionary;
...
/**
    ...
    However, you should keep in mind the guidelines provided in @ref
   when_to_free_a_dictionary.
...
/**
    ...
    @param	key		The name of the key used to index the new value.
                    ...
                    then the existing value associated with the key will be
   freed prior to the new value's assignment.
    @param	value	The new value to append to the dictionary.
    @return			A Max error code.
*/
t_max_err dictionary_appendlong(t_dictionary* d, t_symbol* key,
                                t_atom_long value);
...
/**
   ...
   freed prior to the new value's assignment.
    ...
    @return			A Max error code.
*/
t_max_err dictionary_appendsym(t_dictionary* d, t_symbol* key,
                               t_symbol* value);
...
// private
t_max_err dictionary_appendattribute(t_dictionary* d, t_symbol* key,
                                     t_symbol* attrname, t_object* obj);


/**
    Add a C-string to the dictionary.  Internally this uses the #t_string
   object. It is useful to use the #t_string in dictionaries rather than the
   #t_symbol to avoid bloating Max's symbol table unnecessarily.
   ...
   freed prior to the new value's assignment.
    ...
    @return			A Max error code.
*/
t_max_err dictionary_appendstring(t_dictionary* d, t_symbol* key,
                                  const char* value);


/**
    Add an array of atoms to the dictionary.
    Internally these atoms will be copied into a #t_atomarray object, which
   will be appended to the dictionary with the given key.
   ...
   freed prior to the new value's assignment.
    ...
    @param	argv	The address of the first atom in the array to append to the
   dictionary.
    @return			A Max error code.
*/
...
// internal use only
t_max_err dictionary_appendatoms_flags(t_dictionary* d, t_symbol* key,
                                       long argc, t_atom* argv, long flags);


/**
    Add an @ref atomarray object to the dictionary.
    Note that from this point on that you should not free the #t_atomarray*,
   because the atomarray is now owned by the dictionary, and freeing the
   dictionary will free the atomarray as discussed in @ref
   when_to_free_a_dictionary.
   ...
   freed prior to the new value's assignment.
    ...
    @return			A Max error code.
*/
t_max_err dictionary_appendatomarray(t_dictionary* d, t_symbol* key,
                                     t_object* value);


/**
    Add a dictionary object to the dictionary.
    Note that from this point on that you should not free the #t_dictionary*
   that is being added, because the newly-added dictionary is now owned by the
   dictionary to which it has been added, as discussed in @ref
   when_to_free_a_dictionary.
   ...
   freed prior to the new value's assignment.
    ...
    @return			A Max error code.
*/
t_max_err dictionary_appenddictionary(t_dictionary* d, t_symbol* key,
                                      t_object* value);


/**
    Add an object to the dictionary.
    Note that from this point on that you should not free the #t_object* that
   is being added, because the newly-added object is now owned by the
   dictionary to which it has been added, as discussed in @ref
   when_to_free_a_dictionary.
   ...
   freed prior to the new value's assignment.
    ...
    @return			A Max error code.
*/
t_max_err dictionary_appendobject(t_dictionary* d, t_symbol* key,
                                  t_object* value);


t_max_err dictionary_appendobject_flags(t_dictionary* d, t_symbol* key,
                                        t_object* value, long flags);
...
/**
    ...
    @param	value	The address of variable to hold the value associated with
   the key.
    @return			A Max error code.
*/
t_max_err dictionary_getlong(C74_CONST t_dictionary* d, t_symbol* key,
                             t_atom_long* value);
...
/**
   ...
   the key.
    @return			A Max error code.
*/
t_max_err dictionary_getfloat(C74_CONST t_dictionary* d, t_symbol* key,
                              double* value);
...
/**
   ...
   the key.
    @return			A Max error code.
*/
t_max_err dictionary_getsym(C74_CONST t_dictionary* d, t_symbol* key,
                            t_symbol** value);
...
/**
   ...
   the key.
    @return			A Max error code.
*/
t_max_err dictionary_getatom(C74_CONST t_dictionary* d, t_symbol* key,
                             t_atom* value);


// private
t_max_err dictionary_getattribute(const t_dictionary* d, t_symbol* key,
                                  t_symbol* attrname, t_object* obj);


/**
    Retrieve a C-string pointer from the dictionary.
    The retrieved pointer references the string in the dictionary, it is not a
   copy.
...
/**
    ...
    To fetch a copy of the t_atoms from the dictionary, use
   dictionary_copyatoms().

    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	key		The key associated with the value to lookup.
    @param	argc	The address of a variable to hold the number of atoms in
   the array.
    @param	argv	The address of a variable to hold a pointer to the first
   atom in the array.
    @return			A Max error code.
...
/**
   ...
   dictionary_copyatoms().
    ...
    @param	d		The dictionary instance.
    @param	key		The key associated with the value to lookup.
    @param	stringstosymbols		The flag to convert strings to symbols
   (true,false).
    @param	argc	The address of a variable to hold the number of atoms in
   the array.
    @param	argv	The address of a variable to hold a pointer to the first
...
*/
t_max_err dictionary_getatoms_ext(const t_dictionary* d, t_symbol* key,
                                  long stringstosymbols, long* argc,
                                  t_atom** argv);

/**
    Retrieve copies of a #t_atom array in the dictionary.
    The retrieved pointer of t_atoms in the dictionary has memory allocated and
   copied to it from within the function. You are responsible for freeing it
   with sysmem_freeptr().
   ...
   the array.
    @param	argv	The address of a variable to hold a pointer to the first
   atom in the array. You should initialize this pointer to NULL prior to
   passing it to dictionary_copyatoms().
    @return			A Max error code.
...
/**
    Retrieve the address of a #t_atom array of in the dictionary within nested
   dictionaries. The address can index into nested dictionaries using the '::'
   operator.  For example, the key "field::subfield" will look for the value at
   key "field" and then look for the value "subfield" in the value found at
   "field".
    ...
    @param	errstr	An error message if an error code was returned. Optional,
   pass NULL if you don't want it.
    @return			A Max error code.
*/
t_max_err dictionary_get_ex(t_dictionary* d, t_symbol* key, long* ac,
...
/**
   ...
   the key.
    @return			A Max error code.
*/
t_max_err dictionary_getobject(C74_CONST t_dictionary* d, t_symbol* key,
                               t_object** value);


/**
    Test a key to set if the data stored with that key contains a #t_string
   object.

    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	key		The key associated with the value to test.
    @return			Returns true if the key contains a #t_string, otherwise
   returns false.
*/
long dictionary_entryisstring(C74_CONST t_dictionary* d, t_symbol* key);
...
/**
    Test a key to set if the data stored with that key contains a #t_atomarray
   object.

    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	key		The key associated with the value to test.
    @return			Returns true if the key contains a #t_atomarray, otherwise
   returns false.
*/
long dictionary_entryisatomarray(C74_CONST t_dictionary* d, t_symbol* key);
...
/**
    Test a key to set if the data stored with that key contains a #t_dictionary
   object.

    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	key		The key associated with the value to test.
    @return			Returns true if the key contains a #t_dictionary, otherwise
   returns false.
*/
long dictionary_entryisdictionary(C74_CONST t_dictionary* d, t_symbol* key);
...
/**
    ...
    The dictionary_getkeys() method will allocate memory for the keys it
   returns. You are then responsible for freeing this memory using
   dictionary_freekeys(). <em>You must use dictionary_freekeys(), not some
   other method for freeing the memory.</em>

    @ingroup 	dictionary
    @param		d		The dictionary instance.
    @param		numkeys	The address of a long where the number of keys
   retrieved will be set.
    @param		keys	The address of the first of an array #t_symbol pointers
   where the retrieved keys will be set.
    @return				A max error code.

    @remark		The following example demonstrates fetching all of the keys
   from a dictionary named 'd' in order to iterate through each item stored in
   the dictionary.
    @code
    t_symbol	**keys = NULL;
    long		numkeys = 0;
...
*/
t_max_err dictionary_getkeys(C74_CONST t_dictionary* d, long* numkeys,
                             t_symbol*** keys);
t_max_err dictionary_getkeys_ordered(C74_CONST t_dictionary* d, long* numkeys,
                                     t_symbol*** keys);
...
/**
    ...
    If freeing the object is inappropriate or undesirable, use
...
t_max_err dictionary_chuckentry(
    t_dictionary* d,
    t_symbol* key); // remove a value from the dictionary without deleting it


/**
    Delete all values from a dictionary.
    This method will free the objects in the dictionary.
    If freeing the objects is inappropriate or undesirable then you should
...
t_dictionary* dictionary_clone(t_dictionary* d);
t_max_err dictionary_clone_to_existing(const t_dictionary* d,
                                       t_dictionary* dc);
t_max_err dictionary_copy_to_existing(const t_dictionary* d, t_dictionary* dc);
t_max_err dictionary_merge_to_existing(const t_dictionary* d,
                                       t_dictionary* dc);
t_max_err dictionary_copy_nonunique_to_existing(const t_dictionary* d,
                                                t_dictionary* dc);
...
/**
    ...
    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	fun		The function to call, specified as function pointer cast to
   a Max #method.
    @param	arg		An argument that you would like to pass to the function
   being called.

    @remark			The dictionary_funall() method will call your function for
   every entry in the dictionary. It will pass both a pointer to the
   #t_dictionary_entry, and any argument that you provide. The following
   example shows a function that could be called by dictionary_funall().
    @code
...
/**
    ...
    @param	argv	The address of a #t_atom vector to which the values will be
   copied.

    @see dictionary_entry_getkey()
    @see dictionary_funall()
*/
void dictionary_entry_getvalues(t_dictionary_entry* x, long* argc,
                                t_atom** argv);


/**
    Given 2 dictionaries, copy the keys unique to one of the dictionaries to
   the other dictionary.

    @ingroup			dictionary
    @param	d			A dictionary instance.  This will be the destination
   for any values that are copied.
    @param	copyfrom	A dictionary instance from which we will copy any
...
/**
    ...
    @param	value	The address of variable to hold the value associated with
   the key.
    @param	def		The default value to return in the absence of the key
   existing in the dictionary.
    @return			A Max error code.

    @see			dictionary_getlong()
*/
t_max_err dictionary_getdeflong(const t_dictionary* d, t_symbol* key,
...
*/
t_max_err dictionary_getdeffloat(const t_dictionary* d, t_symbol* key,
...
*/
t_max_err dictionary_getdefsym(const t_dictionary* d, t_symbol* key,
                               t_symbol** value, t_symbol* def);
...
*/
t_max_err dictionary_getdefatom(const t_dictionary* d, t_symbol* key,
                                t_atom* value, t_atom* def);
...
*/
t_max_err dictionary_getdefstring(const t_dictionary* d, t_symbol* key,
                                  const char** value, char* def);
...
/**
    ...
    The retrieved pointer references the t_atoms in the dictionary.
    To fetch a copy of the t_atoms from the dictionary, use
   dictionary_copyatoms(). If the named key doesn't exist, then return a
   default array of atoms, specified as a #t_atomarray*.
   ...
   the array.
    @param	argv	The address of a variable to hold a pointer to the first
   atom in the array.
    @param	def		The default values specified as an instance of the
   #t_atomarray object.
    @return			A Max error code.
...
/**
    ...
    The retrieved pointer of t_atoms in the dictionary has memory allocated and
   copied to it from within the function. You are responsible for freeing it
   with sysmem_freeptr(). If the named key doesn't exist, then copy a default
   array of atoms, specified as a #t_atomarray*.
...
/**
    ...
    @ingroup		dictionary
    @param	d		The dictionary instance.
    @param	recurse	If non-zero, the dictionary will be recursively unravelled
   to the Max window. Otherwise it will only print the top level.
    @param	console	If non-zero, the dictionary will be posted to the console
   rather than the Max window. On the Mac you can view this using Console.app.
                    On Windows you can use the free DbgView program which can
...
/**
    ...
    @param	dst		The destination dictionary to which the entries will be
   copied.
    @param	keys	The address of the first of an array of #t_symbol* that
   specifies which keys to copy.
    @return			A Max error code.

    @see			dictionary_copyunique()
*/
t_max_err dictionary_copyentries(t_dictionary* src, t_dictionary* dst,
                                 t_symbol** keys);


/**
    Create a new dictionary populated with values using a combination of
   attribute and sprintf syntax.

    @ingroup		dictionary
    @param	fmt		An sprintf-style format string specifying key-value pairs
   with attribute nomenclature.
    @param	...		One or more arguments which are to be substituted into the
   format string.
    @return			A new dictionary instance.

    @remark			Max attribute syntax is used to define key-value pairs. For
   example,
    @code
    "@key1 value @key2 another_value"
    @endcode

    @remark			One common use of this to create dictionary that represents
   an element of a patcher, or even an entire patcher itself.  The example
   below creates a dictionary that can be passed to a function like
   newobject_fromdictionary() to create a new object.
    @code
    t_dictionary *d;
    char text[4];
...
/**
    ...
    across multiple calls, or holding on to internal dictionary element
   pointers for complex operations.
...
/**
    Release a lock on a dictionary for preventing dictionary lock for
   transactions across multiple calls, or holding on to internal dictionary
   element pointers for complex operations.
...
/**	Read the specified JSON file and return a #t_dictionary object.
 ...
 @param	d			The address of a #t_dictionary pointer that will be set to
...
/**	Serialize the specified #t_dictionary object to a JSON file.
 @ingroup			dictionary
 @param	d			The dictionary to serialize into JSON format and write to
 disk.
