  ...
  public:
        ...
        , m_notification_callback{ a_function }
    {
        if (create_messages) {
            ...
            m_set_meth = std::make_unique<message<>>(&m_owner, "set", "Choose a named buffer~ from which to read.", MIN_FUNCTION {
                        set(args[0]);
                        return {}; });

            m_dblclick_meth = std::make_unique<message<>>(&m_owner, "dblclick", MIN_FUNCTION {
                         max::buffer_view(max::buffer_ref_getobject(m_instance));
                         return {}; });

            m_notify_meth = std::make_unique<message<>>(&m_owner, "notify", MIN_FUNCTION { return handle_notification(&m_owner, args); });
        }
    }
